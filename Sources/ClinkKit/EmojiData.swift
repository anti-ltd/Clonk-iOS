...
public enum EmojiData {
    ...
    /// Inverted index: a whole name-token → the emoji whose names contain it, in
    /// curated order (iconic variant first). Built once so the predictive-bar
    /// lookup is a single dictionary hit instead of re-splitting all ~2,700 names
    /// on every keystroke — `emojiSuggestions` runs on the hot typing path (it's
    /// computed inside the engine's per-word `compute`), so the scan it replaced
    /// was pure waste. Iterating `nameIndex` in order preserves curated order per
    /// token; the per-entry `Set` keeps a name's repeated token from listing the
    /// same emoji twice.
    private static let tokenIndex: [String: [String]] = {
        var index: [String: [String]] = [:]
        for (emoji, name) in nameIndex {
            for token in Set(name.split(separator: " ").map(String.init)) {
                index[token, default: []].append(emoji)
            }
        }
        return index
    }()
    ...
    public static func emojiSuggestions(for word: String, limit: Int = 2) -> [String] {
        ...
        guard let hits = tokenIndex[w] else { return [] }
